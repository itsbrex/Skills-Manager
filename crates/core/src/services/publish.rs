...
fn walk_publish_files(
    ...
    for entry in entries.flatten() {
        ...
        let metadata = fs::metadata(&path)
            .map_err(|e| format!("无法读取文件信息 {}: {}", rel_path, e))?;
        ...
        let bytes =
            fs::read(&path).map_err(|e| format!("无法读取文件 {}: {}", rel_path, e))?;
...
fn max_version(a: Option<&str>, b: Option<&str>) -> Option<String> {
    let parse = |value: Option<&str>| {
        value.and_then(|value| semver::Version::parse(value.trim()).ok())
    };
...
pub async fn publish_skill(
    ...
    let external_url = owner_for_url
...
mod tests {
    ...
    fn sanitize_slug_normalizes_arbitrary_names() {
        ...
        assert_eq!(sanitize_slug("  Git Worktree Cleanup "), "git-worktree-cleanup");
    ...
    fn max_version_prefers_the_higher_known_version() {
        assert_eq!(max_version(Some("1.2.3"), Some("1.3.0")).as_deref(), Some("1.3.0"));
        assert_eq!(max_version(Some("2.0.0"), Some("1.9.9")).as_deref(), Some("2.0.0"));
        // 远端查不到时退回本地记录。
        ...
        // 无法解析的一方被忽略，而不是让整体退化。
        assert_eq!(max_version(Some("garbage"), Some("1.1.0")).as_deref(), Some("1.1.0"));
    }
