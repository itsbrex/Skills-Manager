...
pub async fn translate_skill(
    ...
    translation::translate_skill(&provider, &target_lang, input).await
}
...
pub async fn translate_marketplace_skill(
    ...
    translation::translate_skill(&provider, &target_lang, payload).await
}
...
pub async fn translate_skills_batch(
    ...
    for (idx, instance_id) in instance_ids.iter().enumerate() {
        ...
        match translation::translate_skill(&provider, &target_lang, input).await {
...
fn lookup_skill_cache(
    ...
    let content_md = fs::read_to_string(skill.path.join("SKILL.md")).ok();
...
pub fn get_cached_marketplace_translations(
    ...
    let entries = inputs
        ...
        .map(|input| {
            ...
            let translation = cache.get(&key).map(|hit| SkillTranslationOutput {
                name: hit.name,
                description: hit.description,
                content_md: hit.content_md,
                cached: true,
