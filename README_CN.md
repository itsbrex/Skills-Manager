...
通过强大的**软链接同步机制（Symlink Synchronization）**，您只需编写一次技能，即可在 29+ 款受支持的 AI 工具（包括 Claude Code、Codex、Cursor、Gemini CLI、Windsurf、Trae 等）中即时生效，实现"一处编写，多处使用"。
...
- **🔌 多工具支持**：开箱即用支持 29+ 款 AI 工具（Claude Code、Codex、Cursor、Gemini CLI、Windsurf、Trae、Cline、Augment、Goose 等），并支持自定义扩展。
